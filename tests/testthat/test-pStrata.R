...
partialStrata <- function(ExpressionSet) {
        ...
        partialStrata <-
                matrix(NA_real_, ncol = ncols - 2, nrow = length(names(table(
                        ExpressionSet[, 1]
                ))))
        for (j in 1:length(names(table(ExpressionSet[, 1])))) {
                partialStrata[j,] <-
                        colSums(partialTXI[which(ExpressionSet[, 1] == j) ,])
        }
...
test_that("is.ExpressionSet() throughs error when no ExpressionSet is entered to pStrata()",
          {
                  expect_error(
                          pStrata(nonStandardExpressionSet)
                  )
          })
...
test_that("pStrata() computes correct partial TAI values ", {
        expect_true(equal_df(
                ...
                partialStrata(PhyloExpressionSetExample)
        ))
})

test_that("pStrata() computes correct partial TDI values ", {
        expect_true(equal_df(
                pStrata(DivergenceExpressionSetExample),
                partialStrata(DivergenceExpressionSetExample)
        ))
})
...
test_that("colSums(pStrata(ExpressionSet)) == TAI(ExpressionSet)", {
        ...
        expect_equal(colSums(pStrata(DivergenceExpressionSetExample)),
