test_that("top_variance_genes works", {
    # Test default parameters
    top_var_genes <- top_variance_genes(example_phyex_set)
    ...
    top_var_90 <- top_variance_genes(example_phyex_set, p = 0.9)
    top_var_95 <- top_variance_genes(example_phyex_set, p = 0.95)
    expect_true(length(top_var_90) >= length(top_var_95))
    
    # Test with very low percentile
    top_var_50 <- top_variance_genes(example_phyex_set, p = 0.5)
    expect_true(length(top_var_50) >= length(top_var_90))
})

test_that("top_expression_genes works", {
    # Test default parameters
    top_expr_genes <- top_expression_genes(example_phyex_set)
    ...
    # Test with different percentile
    top_expr_90 <- top_expression_genes(example_phyex_set, p = 0.9)
    top_expr_95 <- top_expression_genes(example_phyex_set, p = 0.95)
    expect_true(length(top_expr_90) >= length(top_expr_95))
})

test_that("lowly_expressed_genes works", {
    # Test default threshold
    lowly_expr_genes <- lowly_expressed_genes(example_phyex_set)
    expect_true(is.character(lowly_expr_genes))
    expect_true(all(lowly_expr_genes %in% example_phyex_set@gene_ids))
    
    # Test with different threshold
    lowly_expr_5 <- lowly_expressed_genes(example_phyex_set, threshold = 5)
    lowly_expr_10 <- lowly_expressed_genes(example_phyex_set, threshold = 10)
    expect_true(length(lowly_expr_5) <= length(lowly_expr_10))
    
    # Test with very high threshold (should return most genes)
    lowly_expr_high <- lowly_expressed_genes(example_phyex_set, threshold = 1000)
...
test_that("Gene selection functions return valid genes", {
    ...
    top_var <- top_variance_genes(example_phyex_set, p = 0.8)
    top_expr <- top_expression_genes(example_phyex_set, p = 0.8)
    lowly_expr <- lowly_expressed_genes(example_phyex_set, threshold = 5)
...
test_that("Gene selection with extreme parameters", {
    ...
    top_var_0 <- top_variance_genes(example_phyex_set, p = 0)
    ...
    top_var_99 <- top_variance_genes(example_phyex_set, p = 0.99)
    expect_true(length(top_var_99) <= 1000)  # Should be small subset
    
    # Test with threshold = 0 (should return very few genes)
    lowly_0 <- lowly_expressed_genes(example_phyex_set, threshold = 0)
