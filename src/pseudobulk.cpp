...
//' @title Create Pseudobulk Expression Matrix (Cell-Parallel Implementation)
//' @description Alternative implementation that parallelizes over cells instead of genes,
//' which can be more efficient for datasets with many cells but fewer genes.
//' 
//' @param expression Sparse expression matrix (genes x cells) - dgCMatrix format
//' @param groups Integer vector indicating group membership for each cell (0-based indexing)
//' @param n_groups Integer, total number of unique groups
...
//' 
//' @details
//' This implementation parallelizes over cells, making it suitable for datasets
//' with many cells but relatively few genes. It accumulates results using atomic
//' operations to avoid race conditions.
//' 
//' @keywords internal
// [[Rcpp::export]]
arma::mat cpp_pseudobulk_cell_parallel(const arma::sp_mat& expression,
                                       const arma::uvec& groups,
                                       int n_groups,
                                       int ncores = 10) {
    ...
    RcppThread::parallelFor(0, n_cells, [&](int j) {
        ...
        for (; col_it != col_end; ++col_it) {
            ...
            #pragma omp atomic
            result(gene_idx, group_idx) += expr_val;
        }
