...
as backround set to perform enrichment analyses with `PlotEnrichment()`.
...
Here, the first argument is either a standard `ExpressionSet` object (in case `use.only.map = FALSE`: default) or a `Phylostratigraphic Map` or `Divergence Map` (in case `use.only.map = TRUE`; see [Introduction](https://github.com/HajkD/myTAI/blob/master/vignettes/Introduction.Rmd) for details). The second argument `test.set` specifies the gene ids also stored in the corresponding `ExpressionSet` or `Phylostratigraphic Map/ Divergence Map` for which enrichment shall be quantified and visualized.
...
As you noticed, when performing `PlotEnrichment()` function, the p-values and the
enrichment matrix (storing $C_1$ and $C_2$).
...
In case you are only interested in the p-values of the Fisher test and the enrichment matrix without illustrating the barplot,
you can specify the `plot.bars = FALSE` argument to only retrieve the numeric results.
...
since in most cases the `test.set` is an subset of the input `ExpressionSet` or `Phylostratigraphic Map`/`Divergence Map` one could also specify the `complete.bg` argument to remove all `test.set` genes from the background set when performing the Fisher test abd visualization.

The following two examples allow you to compare the results when retaining all genes as backround set compared with the option to remove `test.set` genes from the background set.
...
As you can see, although some p-values change values, the qualitative result did not change.
...
because both options are valid and have advantages and disatvantages and depend on a valid interpretation.
...
overrepresented in the set of brain specific genes. 
...
Again, we retrieve the _D. rerio_ specific taxonomy represented by PS1-14 using the [biomartr](https://github.com/HajkD/biomartr) package (see [Introduction](https://github.com/HajkD/myTAI/blob/master/vignettes/Introduction.Rmd) and [Taxonomy](https://github.com/HajkD/biomartr/blob/master/vignettes/Taxonomy.Rmd) for details).
...
Sestak and Domazet-Loso, 2015 callapsed these 28 taxonomic nodes into 14 taxonomic nodes
...
Based on the phylostrata categorization of Sestak and Domazet-Loso, 2015, PS4 represents `Holozoa (= Metazoa + allies)`, PS5 represents `Metazoa`, and PS7 represents `Bilateria`. 

Now, the overrepresentation results of brain specific genes returned by `PlotEnrichment()`
provide evidence, that brain specific genes might indeed have originated during the emergence of the nervous system at the metazoan–eumetazoan transition leading to the interpretation that the vertebrate brain has a stepwise adaptive history where most of its extant organization was already present in the chordate ancestor as argued by Sestak and Domazet-Loso, 2015.
...
In case you also have an `ExpressionSet` storing the `Phylostratigraphic Map` of _D. rerio_
as well as an expression set, you can use the `PlotGeneSet()` function implemented in `myTAI` to furthermore, visualize the expression levels of brain specific genes which 
...
The greatest advantage of `Phylostratum` and `Divergence Stratum` enrichment analyses can be unfolded
when it is based on the functional annotation of the `test.set`. The [Functional Annotation](https://github.com/HajkD/biomartr/blob/master/vignettes/Functional_Annotation.Rmd) and [Phylotranscriptomics](https://github.com/HajkD/biomartr/blob/master/vignettes/Phylotranscriptomics.Rmd) vignettes of our [biomartr](https://github.com/HajkD/biomartr) package will show you how to retrieve functional
annotation for a subset of genes. Categorizing genes into common functional groups or processes via `biomartr` will then allow you to search for genes deriving from enriched `Phylostrata` or `Divergence Strata`. The [Introduction](https://github.com/HajkD/myTAI/blob/master/vignettes/Introduction.Rmd) vignette showed you how `Phylostrata` or `Divergence Strata` can be linked with the origin of evolutionary events.
This correlation between the predicted origin of genes (`Phylostratigraphic Map`) or its
current state of selection between closely related species (`Divergence Map`) and their
functional annotation allows you to detect signals of potential evolutionary origins of
specific biological functions, processes, or tissues and their active maintenance between
