...
In case you are only interested in the p-values of the Fisher test and the enrichment matrix without illustrating the bar plot,
...
The following two examples allow you to compare the results when retaining all genes as background set compared with the option to remove `test.set` genes from the background set.
...
Again, we retrieve the _D. rerio_ specific taxonomy represented by PS1-14 using the `taxonomy()` funtion (see [Introduction](https://github.com/HajkD/myTAI/blob/master/vignettes/Introduction.Rmd) and [Taxonomy](https://github.com/HajkD/myTAI/blob/master/vignettes/Taxonomy.Rmd) for details).
...
Additionally, the `SelectGeneSet()` function allows you to obtain the `ExpresisonSet` subset of selected genes (`gene.set`) for subsequent analyses.
...
All methods implemented in `?p.adjust` can be specified:
