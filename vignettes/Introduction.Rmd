...
The first step is to install the [orthologr](https://github.com/HajkD/orthologr#installation-guide) package and the [BLAST command line tool](https://github.com/HajkD/orthologr/blob/master/vignettes/Install.Rmd#install-blast) on your machine. In the following example we will compute a `Divergence Map` for _A. thaliana_ versus _A. lyrata_.
...
__Since version 0.4.0 myTAI now includes its own DivergenceStratigraphy() function which is based on the divergence_stratigraphy() function implemented in `orthologr`!__
...
# Since myTAI v. 0.4.0
# Compute the divergence map of A. thaliana vs. A. lyrata
Ath_vs_Aly_DM <- DivergenceStratigraphy(
                         query_file      = "Athaliana_167_cds.fa",
                         subject_file    = "Alyrata_107_cds.fa",
                         eval            = "1E-5", 
                         ortho_detection = "RBH",
...
For a more detailed tutorial on how to use the `divergence_stratigraphy()` function implemented in [orthologr](https://github.com/HajkD/orthologr/blob/master/vignettes/divergence_stratigraphy.Rmd) please consult the [Divergence Stratigraphy Vignette](https://github.com/HajkD/orthologr) and for learning about the new functionality of
the `DivergenceStratigraphy()` function implemented in `myTAI` please consult the [Stratigraphy Vignette](Stratigraphy.Rmd).
