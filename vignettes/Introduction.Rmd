...
A series of tutorials aim to guide you through the `myTAI` package: 
...
* __3) Advanced:__ Vignette - Data Retrieval and Divergence Stratigraphy using the [orthologr](https://github.com/HajkD/orthologr) package
...
So, the first step in performing phylotranscriptomic analyses is to
...
The following code illustrates how a typical `Phylostratigraphic Map` and `Divergence Map` are structured:
...
You can find a `Perl Script` for performing Phylostratigraphy in [Drost et al., 2015](https://github.com/HajkD/Active-maintenance-of-phylotranscriptomic-hourglasses/blob/master/README.md). 
...
3) Perform BLAST homology searches using all protein sequences of the query organism (organism of interest) against a curated data dase.
...
to run the `Phylostratigraphy` algorithm. Now please install the [biomartr](https://github.com/HajkD/biomartr#installation-guide) package to be able to automatically define a taxonomy for _A. thaliana_. After installing `biomartr` on your system, you can retrieve taxonomic classification information for any annotated species by typing (here we use _A. thaliana_ as example): 
...
In some cases (due to the coverage of sequenced genomes and predicted proteomes) only a very small number of proteomes are classified into a specific taxonomic category. In this case, taxonomic categories can be summarized. For example, Quint et al., 2012 and Drost et al., 2015 summarized the taxonomic categories shown above into 12 categories, so that a minimum amount of proteomes represent a corresponding 
taxonomic category (see [Fig. 1 c](http://mbe.oxfordjournals.org/content/32/5/1221/F1.expansion.html)). These 12 taxonomic categories are respresented as 12 `phylostrata` starting with the _oldest_ `phylostratum` (= `PS1` = `cellular organisms`) up to the _youngest_ `phylostratum` (= `PS12` = `A. thaliana`).
...
Here, all genes correspond to `Phylostratum 1` (PS1) representing the taxonomic category `cellular organisms`.


The `taxonomy()` functions allows you to retrieve taxonomic classification information for any annotated organism of interest by specifying its `scientific name`, e.g. `Arabidopsis thaliana`, `Homo sapiens`, `Mus musculus`, etc. In case you would like to add more information to the underlying taxonomy, please consult the  [Taxonomy](https://github.com/HajkD/biomartr/blob/master/vignettes/Taxonomy.Rmd#taxonomic-information-retrieval) Vignette of the [biomartr](https://github.com/HajkD/biomartr#installation-guide) package.
...
2) Download the sequence database [phyloBlastDB_Drost_Gabel_Grosse_Quint.fa](http://msbi.ipb-halle.de/download/phyloBlastDB_Drost_Gabel_Grosse_Quint.fa.tbz) used for BLAST searches and unpack it (`tar xfvj phyloBlastDB_Drost_Gabel_Grosse_Quint.fa.tbz`).
...
1) Orthology Inference using BLAST reciprocal best hit based on `blastp` algorithm
...
In case you are working on a multicore machine, you can specify the `comp_cores` argument to perform `divergence_stratigraphy()` using multiple computing cores. 
...
All functions implemented in the `myTAI` package expect a specific data format as input data. These standardized data formats are referred to as __PhyloExpressionSet__ and __DivergenceExpressionSet__. The following sections will give you an overview about these data formats.
...
The function `MatchMap()` allows you to join a `Phylostratigraphic Map` with an `ExpressionMatrix` to obtain a `PhyloExpressionSet`. In some cases, the GeneIDs stored in the `ExpressionMatrix` and in the `Phylostratigraphic Map` do not match. This is due to GeneID mappings between different data bases and annotations. To map non matching GeneIDs between data bases and annotations, please consult the [Functional Annotation Vignette](https://github.com/HajkD/biomartr/blob/master/vignettes/Functional_Annotation.Rmd) in the [biomartr](https://github.com/HajkD/biomartr) package. The `biomartr` package allows you to map GeneIDs between data base annotations.
...
The following `DivergenceExpressionSet` example shows the standard `DivergenceExpressionSet` data set format.
...
procedure to download published datasets. In this example we will download `Supplementary Table S3` from Drost et al., 2015. A list of published `Phylostratigraphic Maps` and `Divergence Maps` can be found [here](https://github.com/HajkD/published_phylomaps).
...
You can also use already published `Phylostratigraphic Maps` and `Divergence Maps` that can be found [here](https://github.com/HajkD/published_phylomaps) and match them with your transcriptome data of interest using the `MatchMap()` function. This procedure allows you to generate your own `PhyloExpressionSet` and `DivergenceExpressionSet` to perform phylotranscriptomic analyses with `myTAI`.
...
Analogous to the TAI measure, the _Transcriptome Divergence Index_ (TDI) was introduced by Quint et al., 2012 and Drost et al., 2015 as weighted arithmetic mean of the degree of sequence divergence of the transcriptome being expressed during a corresponding developmental stage $s$.
...
To make sure that correct modules have been selected to perform the `ReductiveHourglassTest`, you can use the `shaded.area` argument to visualize chosen modules:
...
Another way to visualize the cumulative contribution of each `Phylostratum` or `Divergence Stratum` to the global Transcriptome Age Index or Transcriptome Divergence Index profile was introduced by Domazet-Loso and Tautz, 2010 (Fig. 1b). This `Phylostratum` or `Divergence Stratum` specific contribution on the global TAI or TDI pattern can be visualized using:

#### Example: `Phylostrata`

```{r,eval = FALSE}
# visualize phylostrata contribution to the global TAI pattern
 PlotContribution(PhyloExpressionSetExample, legendName = "PS", lwd = 6)
```

The exact values of the `Phylostratum` specific cumulative TAI values can be obtained
...
# visualize divergence stratum contribution to global TDI
 PlotContribution(DivergenceExpressionSetExample, legendName = "DS", lwd = 6)
```
