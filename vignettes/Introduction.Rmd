...
The `taxonomy()` function allows users to retrieve taxonomic classification information for any annotated organism of interest by specifying its `scientific name`, e.g. `Arabidopsis thaliana`, `Homo sapiens`, `Mus musculus`, etc. In case you would like to retrieve more information about each functionality and database connection implemented in the `taxonomy()`  function please consult the  [Taxonomy](Taxonomy.html) Vignette.
