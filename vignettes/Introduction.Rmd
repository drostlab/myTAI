...
Due to the coverage of sequenced genomes and predicted proteomes in some cases only a very small number of proteomes are classified into a specific taxonomic category. In this case, taxonomic categories can be summarized. For example, [Quint et al., 2012](http://www.nature.com/nature/journal/v490/n7418/full/nature11394.html) and [Drost et al., 2015](http://mbe.oxfordjournals.org/content/32/5/1221.full) summarized the taxonomic categories shown above into 12 categories, so that a minimum amount of proteomes represent a corresponding 
...
This representation of taxonomic categories as `phylostrata` labeled as discrete values _1, 2, ..., n_  is stored in all `Phylostratigraphic Maps`.
...
Now the resulting `Divergence Map` stored the `divergence stratum` in the first column and the corresoponding `gene id` in the second column.
...
A `PhyloExpressionSet` specifies the combination of a `Phylostratigraphic Map` with an `Gene Expression Set`. Hence, the to generate a `PhyloExpressionSet` the following two tables need to be joined:
