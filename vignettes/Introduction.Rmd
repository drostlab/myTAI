...
The function `MatchMap()` allows you to join a `Phylostratigraphic Map` with an `ExpressionMatrix` to obtain a `PhyloExpressionSet`.
...
A `DivergenceExpressionSet` specifies the combination of a `Divergence Map` with an `Gene Expression Set`. A `DivergenceExpressionSet` can be generated analogous to a `PhyloExpressionSet` by joining a `Divergence Map` with an `ExpressionMatrix` using the `MatchMap()` function.
...
procedure to download published datasets. In this example we will download `Supplementary Table S3` from [Drost et al., 2015](http://mbe.oxfordjournals.org/content/early/2015/03/05/molbev.msv012.full.pdf).
