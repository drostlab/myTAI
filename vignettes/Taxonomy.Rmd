...
This vignette will introduce you to the retrieval of taxonomic information with `myTAI`. 
Most taxonomy functions implemented in `myTAI` rely on the powerful package [taxize](http://cran.r-project.org/web/packages/taxize/index.html).
...
according to phylogenetic classification into Phylostrata (Phylostratigraphy) or to retrieve species specific taxonomic information when performing Divergence Stratigraphy (see [Introduction](https://github.com/HajkD/myTAI/blob/master/vignettes/Introduction.Rmd) for details).
...
* `db = "eol"` : Encylopedia of Life
* `db = "tnrs"` : Taxonomic Name Resolution Service
* `db = "itis"` : Integrated Taxonomic Information Service
* `db = "phylomatic"` : Phylomatic
* `db = "ubio"` : uBio
* `db = "gnr"` : Global Names Resolver
* `db = "gni"` : Global Names Index
* `db = "iucn"` : IUCN Red List
* `db = "tp"` : Tropicos
* `db = "plantminer"` : Plantminer
* `db = "col"` : Catalogue of Life
* `db = "ncbi"` : National Center for Biotechnology Information
* `db = "vascan"` : CANADENSYS Vascan name search API
* `db = "ipni"` : International Plant Names Index (IPNI)
* `db = "bold"` : Barcode of Life Data Systems (BOLD)
* `db = "nbn"` : National Biodiversity Network (UK)
