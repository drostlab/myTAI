...
Genomic phylostratigraphy is a method to date the evolutionary emergence of a given gene (i.e. gene age inference), typically done for all genes in a genome. In practice, one performs homology searches against a evolutionarily-informed database.
...
The crux of the discussion focuses on the homology search bias, though several studis have also noted database bias (e.g., species representation and contamination - see [Barrera-Redondo et al., 2023](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-023-02895-z) for discussion) and evolutionary assumptions (e.g., Dollo parsimony - see [Gàlvez-Morante et al., 2024](https://academic.oup.com/gbe/article/16/4/evae062/7633882)).

#### Homology search bias

The first phylostratigraphy studies used `BLASTp`, which performs pairwise protein sequence alignment, for the homology search step. At the time of these study (late-2000s), `BLASTp` was chosen as it balanced the requirements of sensitivity and speed when performing sequence homology searches against (at that time) a large databases.
...
<details>
  <summary>**Gene age underestimation for genes with certain properties with `BLASTp`**</summary>
...
<details>
  <summary>**Overlooked technical aspects of `BLASTp`**</summary>
...
<details>
  <summary>**Inferring gene age using orthogroups**</summary>
...
*Dollo parsimony*, which precludes the possibility that an identical character (gene) can be gained more than once, is a major conceptual assumption for gene age inference [Gàlvez-Morante et al., 2024](https://academic.oup.com/gbe/article/16/4/evae062/7633882).
...
The gene age inference software, `GenEra`, accounts for potential contamination to some extent. Upstream software can be employed to further address potential contamination but these are typically applied to query genomes and not the database (see [Bálint et al., 2024](https://www.nature.com/articles/s41467-024-45024-5) and [Nevers et al., 2025](https://www.nature.com/articles/s41587-024-02147-w)).
