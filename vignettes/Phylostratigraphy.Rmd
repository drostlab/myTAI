...
With `DIAMOND`, users can overcome the overlooked technical aspects of BLAST such as the `max_target_seqs` issue by setting `--max-target-seqs` to `0`. Furthermore, the `--ultra-sensitive` mode can be employed, which is as sensitive as BLAST while being 80x faster (see [Fig 1. in the DIAMOND paper](https://www.nature.com/articles/s41592-021-01101-x/figures/1)).
