...
combine the previously intorduced techniques to  ... by performing _classic_ gene expression analyses in an evolutionary context. 

In other words, phylotranscriptomics allows us to detect stages or periods of evolutionary conservation and is able to predict the evolutionary orgin of process or trait specific genes
...
functional processes. Then, performing gene expression analyses on these process or trait specific genes allows to provide evidence for potential causes of stage/period specific evolutionary transcriptome conservation. 
...
Usually, one would expect that variation in replicate values is smaller than variation between developmental stages. In this example however we constructed replicate values that
...
The `myTAI` package implements a customized function `CollapseReplicates()` to collapse
replicates stored in a standard `PhyloExpressionSet` or `DivergenceExpressionSet` object.
...
are present, one could specify  `nrep = c(2,3,2)` defining the case that developmental stage 1 stores 2 replicates, stage 2 stores 3 replicates, and stage 3, again stores 2 replicates.
