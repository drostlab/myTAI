...
__Note__ that when using `DiffGenes()` it is assumed that your input dataset has been normalized before passing it to `DiffGenes()`. For RNA-Seq data `DiffGenes()` assumes that the libraries have been normalized to have the same size, i.e., to have the same expected column sum under the null hypothesis.
...
```
  Phylostratum      GeneID      S1->S2     S1->S3      S2->S1     S2->S3     S3->S1     S3->S2
1            1 at1g01040.2 0.027832572 0.04020203 0.027832572 0.13481563 0.04020203 0.13481563
2            1 at1g01050.1 0.852379466 0.31471871 0.852379466 0.36326955 0.31471871 0.36326955
3            1 at1g01070.1 0.003200692 0.00113536 0.003200692 0.02236621 0.00113536 0.02236621
4            1 at1g01080.2 0.086426813 0.03092924 0.086426813 0.45999438 0.03092924 0.45999438
5            1 at1g01090.1 0.090387087 0.04638872 0.090387087 0.04978092 0.04638872 0.04978092
```
...
```
  Phylostratum      GeneID     S1->S2    S1->S3     S2->S1    S2->S3    S3->S1    S3->S2
1            1 at1g01040.2 0.06958143 0.0579859 0.06958143 0.2246927 0.0579859 0.2246927
2            1 at1g01050.1 0.85237947 0.3147187 0.85237947 0.4540869 0.3147187 0.4540869
3            1 at1g01070.1 0.01600346 0.0056768 0.01600346 0.1118311 0.0056768 0.1118311
4            1 at1g01080.2 0.11298386 0.0579859 0.11298386 0.4599944 0.0579859 0.4599944
5            1 at1g01090.1 0.11298386 0.0579859 0.11298386 0.1244523 0.0579859 0.1244523
```
...
you to obtain only significant differentially expressed genes.
...
```
 Phylostratum      GeneID     S1->S2    S1->S3     S2->S1    S2->S3    S3->S1    S3->S2
3            1 at1g01070.1 0.03200692 0.0113536 0.03200692 0.2192432 0.0113536 0.2192432
```

In this example, only 1 out of 10 genes fulfills the p-value criteria (`alpha`) in at least one stage comparison.

#### Rank top p-values

Finally, you can rank genes in increasing p-value order for each stage comparison by typing:
...
empirical distribution parameters. _Nonparametric_ tests are useful when sample populations
...
provided by the [edgeR](http://www.bioconductor.org/packages/release/bioc/html/edgeR.html) package. Hence, before using the negative binomial models in `DiffGenes()` you need to install 
the edgeR package.
...
The detection of DEGs using generalized linear models is based on the powerful implementations
provided by the [edgeR](http://www.bioconductor.org/packages/release/bioc/html/edgeR.html) package. Hence, before using the negative binomial models in `DiffGenes()` you need to install 
the edgeR package.
...
```
  Phylostratum      GeneID      S1->S2     S1->S3      S2->S1     S2->S3     S3->S1     S3->S2
1            1 at1g01040.2 0.060303051 0.06030305 0.060303051 0.13481563 0.06030305 0.13481563
2            1 at1g01050.1 0.852379466 0.54490432 0.852379466 0.54490432 0.54490432 0.54490432
3            1 at1g01070.1 0.004801037 0.00340608 0.004801037 0.02236621 0.00340608 0.02236621
4            1 at1g01080.2 0.129640220 0.09278773 0.129640220 0.45999438 0.09278773 0.45999438
5            1 at1g01090.1 0.090387087 0.07467138 0.090387087 0.07467138 0.07467138 0.07467138
```
