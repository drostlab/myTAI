...
In some cases (high variability of replicates) it might be useful to varify that there is no sequence of replicates (for all possible combination of replicates) that results in a non-significant `TAI` or `TDI` pattern, when the initial pattern with combined replicates was shown to be significant.
...
The p-value vector returned by this function can then be used to plot the p-values to see whether an critical value α is exeeded or not (e.g. $\alpha = 0.05$).
