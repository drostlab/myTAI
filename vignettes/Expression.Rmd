...
Again, you can adjust p-values by specifying the `p.adjust.method` argument.
...
Exact Tests for Differences between Two Groups of Negative-Binomial Counts based on the
...
`method = "doubletail"`, the number of replicates per stage `nrep`, and the library size to equalize library 
sizes by quantile-to-quantile normalization `lib.size` need to be specified (see also `?equalizeLibSizes`).
