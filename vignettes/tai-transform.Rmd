...
vignette: >
  %\VignetteIndexEntry{tai-transform}
...
:::{.tip}
If you are curious about the exact nature of the transformations used by `myTAI::plot_signature_transformed()` and `myTAI::tf_stability()` you can check the list of functions that were applied via `print(myTAI::COUNT_TRANSFORMS)`.
...
Another aspect of the `myTAI` package is the ability to transform gene age information itself using the `myTAI::tfPS()` function. This function is still under development and will be documented in due time.
