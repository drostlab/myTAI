...
#' index (TEI) similar to the \code{\link{TAI}} introduced by \code{Domazet-Loso & Tautz, 2010}.
...
#' TEIs <- TEI(PhyloExpressionSetExample)
#'
#' @export
#' @author Kristian K Ullrich
  ...
  if (is(ExpressionSet, "Matrix")) {
    ...
    if (ncol(ExpressionSet) > split) {
      ...
      for (i in seq_along(split_start)) {
        ...
        sumx_teisum_tei <-
          rcpp_tei_parallel(es, Phylostratum, threads)
      ...
      sumx_teisum_tei <-
        rcpp_tei_parallel(ExpressionSet, Phylostratum)
      tei <- unlist(sumx_teisum_tei["tei"])
      names(tei) <- colnames(ExpressionSet)
    }
  }
  if (is(ExpressionSet, "data.frame") |
      is(ExpressionSet, "tibble")) {
    if (is.ExpressionSet(ExpressionSet)) {
      Phylostratum <- setNames(ExpressionSet$Phylostratum,
                               ExpressionSet$GeneID)
      ExpressionSet <-
        as(data.matrix(ExpressionSet[, 3:ncol(ExpressionSet)]), "sparseMatrix")
