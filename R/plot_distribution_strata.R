...
plot_distribution_strata <- function(strata,
    ...
    if (!is.factor(strata)) {
        stop("'strata' must be a factor vector. Try providing phyex_set@strata as input.", call. = FALSE)
    }

    df <- data.frame(Stratum=strata, GeneID=names(strata))
    df_selected <- df |> filter(GeneID %in% selected_gene_ids)
    if (!as_log_obs_exp) {
        ggplot(df_selected, aes(y = Stratum, fill=Stratum)) + 
            geom_bar(stat="count", position = "dodge", show.legend = FALSE) + 
            labs(fill="Stratum") +
            ylab("Stratum") + 
            xlab("Gene Count") +
            scale_y_discrete(labels=levels(df$Stratum), drop = FALSE) +
        ...
        ggplot(log_ratio, aes(y=Stratum, 
                              x=log_obs_exp,
                              fill= log_obs_exp)) +
