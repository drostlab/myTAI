#' @title Perform Reductive Early Divergence Test
#' @description The \emph{Reductive Early Divergence Test} aims to statistically evaluate the
...
#' does not follow an early divergence like pattern. A p-value < 0.05 indicates that the corresponding phylotranscriptomics pattern does
#' indeed follow an early divergence (high-high-low) shape.
...
#' @details The \emph{reductive early divergence test} is a permutation test based on the following test statistic. 
#'
...
#' In the case of the \emph{reductive early divergence test} a normal distribution seemed plausible.
#'
...
#' @seealso \code{\link{edScore}}, \code{\link{bootMatrix}}, \code{\link{FlatLineTest}},\code{\link{ReductiveHourglassTest}}, \code{\link{ReverseHourglassTest}}, \code{\link{PlotSignature}}
#' @examples
#' 
#' data(PhyloExpressionSetExample)
#'
#' # perform the early divergence test for a PhyloExpressionSet
...
#' # module 3 = late
#' EarlyDivergenceTest(PhyloExpressionSetExample,
#'                        modules = list(early = 1:2, mid = 3:5, late = 6:7), 
#'                        permutations = 1000)
#'
#' 
#' # use your own permutation matrix based on which p-values (EarlyDivergenceTest)
#' # shall be computed
...
EarlyDivergenceTest <- function(ExpressionSet,
                                  modules            = NULL,
                                  permutations       = 1000, 
                                  lillie.test        = FALSE, 
                                  plotHistogram      = FALSE, 
  ...
  if(any(table(unlist(modules)) > 1))
    stop("Intersecting modules are not defined for the EarlyDivergenceTest.", call. = FALSE)
  
  if(length(modules) != 3)
    stop("Please specify three modules: early, mid, and late to perform the EarlyDivergenceTest.", call. = FALSE)
  ...
  # compute the real early divergence of the observed phylotranscriptomics pattern
  # edScore = early divergence score
  real_edv <- edScore(real_age,early = modules[[1]],mid = modules[[2]],late = modules[[3]])
  ...
  score_vector <- apply(resMatrix, 1 ,edScore,early = modules[[1]],
  ...
  if(plotHistogram){
    ...
    graphics::curve( expr = normDensity,
                     xlim = c(min(score_vector,real_edv),max(score_vector,real_edv)),
                     col  = "steelblue",
                     lwd  = 5,
                     xlab = "Scores",
                     ylab = "Frequency" )
    ...
    # plot a red line at the position where we can find the real ed value
    graphics::abline(v = real_edv, lwd = 5, col = "darkred")
    
    #legend("topleft", legend = "A", bty = "n")
    
    p.vals_vec <- vector(mode = "numeric", length = runs)
    lillie_vec <- vector(mode = "logical", length = runs)
    edt <- vector(mode = "list", length = 3)
    ...
    if(parallel){
      ...
      parallel_results <- foreach::foreach(i              = 1:runs,
                                           ...
                                           .errorhandling = "stop") %dopar% {
                                             ...
                                             data.frame(EarlyDivergenceTest( ExpressionSet = ExpressionSet,
    ...
    if(!parallel){
      ...
      for(i in 1:runs){
        ...
        if(lillie.test){
          ...
          edt <- EarlyDivergenceTest( ExpressionSet = ExpressionSet,
                                        permutations  = permutations,
                                        lillie.test   = TRUE, 
                                        plotHistogram = FALSE,
        ...
        p.vals_vec[i] <- edt$p.value
        
        if(lillie.test)
          lillie_vec[i] <- edt$lillie.test
  ...
  #if(real_edv >= 0)
  pval <- stats::pnorm(real_edv,mean = mu,sd = sigma,lower.tail = FALSE)
  
  #if(real_edv < 0)
  #pval <- pnorm(real_edv,mean=mu,sd=sigma,lower.tail=TRUE)
