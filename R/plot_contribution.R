...
plot_contribution <- function(phyex_set) {
    ...
    strata_labels <- levels(phyex_set@stratas)
    df <- data.frame(contribution) |>
        mutate(Strata = factor(strata_labels, levels = rev(strata_labels))) |>
        tidyr::pivot_longer(-Strata, names_to = "Condition", values_to = "TXI") |>
    ...
    p <- ggplot(df, 
        ...
        scale_fill_manual(values = rev(PS_colours(phyex_set@num_stratas))) +
