...
#' @description This function takes the CDS file of a query organism and performs
...
#' @param subject.id a logical value indicating whether \code{query_id} AND \code{subject_id} should be returned as part of the output \code{Sequence Divergence Map}.
#' @param eval a numeric value specifying the e-value cutoff for BLASTP hit detection.
#' @param ortho.inference a character string specifying the orthology inference method that shall be performed
...
#'  }
#'  
#' @return A data.table storing the Sequence Divergence Map of the query organism.
#' @seealso \code{\link{PlotPattern}}, \code{\link{TDI}},  \code{\link{PlotRE}}, \code{\link{RE}}
#' @export

DivergenceStratigraphy <- function(query, 
                                   subject,
                                   subject.id      = FALSE,
                                   eval            = "1E-5",
                                   ortho.inference = "RBH", 
       ...
       # cat("\n")
       # message(paste0("Starting the Divergence Stratigraphy Algorithm using ",cores," cores ..."))
        
        orthologr::divergence_stratigraphy(query_file      = query, 
                                           subject_file    = subject, 
                                           eval            = eval,
                                           ortho_detection = ortho.inference, 
