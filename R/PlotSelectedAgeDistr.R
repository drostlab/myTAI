...
PlotSelectedAgeDistr <- function(ExpressionSet, gene.set,
        ...
        GeneSubSet.LongFormat[ , 1] <- factor(GeneSubSet.LongFormat[ , 1], ordered = TRUE)
        plot.res <- ggplot2::ggplot(GeneSubSet.LongFormat, ggplot2::aes(x = AgeCategory, y = GeneCount)) + ggplot2::geom_bar(stat="identity", fill = col) + ggplot2::labs(x = paste0("\n",legendName), y = ylab.name)  + ggplot2::geom_text(aes( label = GeneCount, y =  GeneCount), stat= "identity", vjust = -.3) + ggplot2::theme_minimal()
